...
**V19.1 NOTE**: This version uses V18.7 database (20,943 facilities: 15,244 SNF + 5,699 ALF), implements corrected scenario calculation methodology (service package differentiation, NOT fee multipliers), and adds comprehensive report documentation with critical corrections (S2 for point values, Fee Structure SOM service breakdown).
...
- Minor fixes: V18.0 â†’ V18.1 â†’ V18.2
- Major changes: V18.x â†’ V19.0
...
Combined Database (Single Source of Truth)
    â†“
Fee Schedule â†’ Economic Model Scenarios (S1, S2, S3)
    â†“
Reports & Validation
```
...
**V19.1 Database Foundation:**
- Based on V18.7 (PowerBI reconciliation + strategic barrier removals)
- SNF: 15,244 facilities (72.8%)
- ALF: 5,699 facilities (27.2%)
- Corporate: 12,053 facilities in 1,026 chains (57.5%)
- Independent: 8,890 facilities (42.5%)
- Served: 1,663 facilities
- Barriers: 848 facilities
...
**Business Impact:**
- **Legal structure â‰  Operational scale** - An LLC operating 1 facility is Independent
...
**Key Insight:** As service packages are enhanced (S1â†’S3), current customer value increases but integration opportunity decreases proportionally.

### 4.8 V19.1 Key Metrics

**Database:** V18.7 (20,943 facilities: 15,244 SNF + 5,699 ALF, 1,663 served, 848 barriers)

**Scenario 1 (Conservative - Baseline Service Packages):**
- Current Revenue: $172.1M
- Integration Revenue: $154.3M
- New Business Revenue: $6,915.6M
- **Total Potential: $7,069.9M**

**Scenario 2 (Market Baseline - PCP Enhanced):**
- Current Revenue: $182.4M
- Integration Revenue: $144.9M  
- New Business Revenue: $6,915.6M (unchanged from S1)
- **Total Potential: $7,060.6M**

**Scenario 3 (Comprehensive - Full Enhancement):**
...
✅ New Business Revenue: IDENTICAL across scenarios ($6.9B)  
✅ Integration Revenue: DECREASES S1→S2→S3  
✅ Current Revenue: INCREASES S1→S2→S3

**Baseline Comparison:**
- V15: 17,434 facilities, $5.24B Total Potential (S1)
- V18: 21,023 facilities, ~$6.9B Total Potential (estimated)
- **V19.1: 20,943 facilities, $7.07B Total Potential (S1 actual)**
...
**CRITICAL NOTE**: Tables 4-6 use **Scenario 2** (market baseline), NOT Scenario 1. S2 represents the most realistic middle-ground projection.
...
**CRITICAL NOTE**: Tables 10-12 use **Scenario 2** (market baseline), NOT Scenario 1.

**Structure: SERVICE BREAKDOWN (NOT State Breakdown)**

These tables show revenue broken down by **service component**, NOT by state.

**Column Structure:**
- Column B-C: PCP ($ and %)
- Column D-E: MH ($ and %)
- Column F-G: CCM ($ and %)
- Column H: Shared Savings ($)
...
2. Filter by Source_Type (SNF/ALF/Total)
3. Calculate revenue by service component:
   - Analyze service mix (PCP Only, MH Only, Integrated)
   - Allocate revenue to PCP, MH, CCM, SS components
   - Calculate percentage distribution
4. Present as Current/Integration/New Biz/Total Potential rows

**CRITICAL**: This is a **service composition analysis**, showing what portion of revenue comes from each service type. It does NOT show geographic/state breakdowns.
...
**Filter:** SOM segment (Corporate only, Existing markets, no barriers) - **Scenario 2**
...
|---------|------|-------------|
| **V19.1** | Nov 2025 | V18.7 database (20,943 facilities), corrected scenario methodology (service packages not fee multipliers), Comprehensive Report fixes (S2 for point values, Fee Structure SOM service breakdown) |
...
| V18.0-V18.6 | Nov 2025 | Iterative data quality improvements |
