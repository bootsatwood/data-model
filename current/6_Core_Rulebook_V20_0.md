...
â”œâ”€ Database specification
â”œâ”€ Fee structure governance
â”œâ”€ Scenario calculation methodology
â”œâ”€ Core QC Protocol
â”‚
REPORT COMPENDIUMS (separate documents)
â”œâ”€ Comprehensive_Report_Compendium_V20.md (6.1)
â”œâ”€ Metro_Market_Analysis_Compendium_V20.md (6.2)
â”œâ”€ [Additional report-specific compendiums]
```
...
Combined Database (Source of Truth)
         â†“
Fee Schedule â”€â”€â†’ Economic Model Scenarios (S1, S2, S3)
                              â†“
...
- Minor fixes: V19.1 â†’ V19.2 â†’ V19.3
- Major changes: V19.x â†’ V20.0
...
**Current Version:** V20.0 (20,943 facilities)
...
| V19.2 | Dec 2025 | Duplicate resolution + data quality fixes | 20,943 | Resolved 9 duplicate records, geocoded 2 facilities, fixed 13 NULL records |
| **V20.0** | Dec 2025 | Cardon barriers + corporate standardization | **20,943** | Added 34 Cardon barriers, standardized 6 corporate names, merged Carrolton entities |
...
#### Rule 1: Blank Corporate Name â†’ Independent
```
IF Corporate_Name IS NULL OR Corporate_Name = ''
  THEN Ownership_Type = "Independent"
```

#### Rule 2: "INDEPENDENT" Placeholder â†’ Independent
```
...
- Integrated_Flag = Yes â†’ PCP_Flag = Yes AND MH_Flag = Yes
- PCP_Flag = Yes, MH_Flag = No â†’ PCP-only facility
- MH_Flag = Yes, PCP_Flag = No â†’ MH-only facility
- All flags = No â†’ Not served
...
**V20.0 Barrier Count:** 891 facilities

**Barrier Breakdown:**
| Category | Count |
|----------|-------|
| Operational barriers | 848 |
| Cardon permanent | 34 |
| Duplicate deactivations | 9 |
| **Total** | **891** |
...
**Supporting Columns:**
- `Distance_to_Metro_Center` â€” Miles from facility to assigned metro center
- `Metro_Center_Used` â€” Which city center was used for distance calculation
...
| MH | 0.50 | Base Ã— 0.50 |
| CCM | 0.30 | Base Ã— 0.30 |
| SS | 0.165 | Base Ã— 0.50 Ã— 0.33 |

### 3.3 Adjusted Service Fees

| Service | SNF Calculation | SNF Result | ALF Calculation | ALF Result |
|---------|-----------------|------------|-----------------|------------|
| PCP | $3,078.00 Ã— 1.00 | $3,078.00 | $2,084.00 Ã— 1.00 | $2,084.00 |
| MH | $1,211.00 Ã— 0.50 | $605.50 | $1,431.00 Ã— 0.50 | $715.50 |
| CCM | $360.00 Ã— 0.30 | $108.00 | $360.00 Ã— 0.30 | $108.00 |
| SS | $4,800.00 Ã— 0.165 | $792.00 | $4,800.00 Ã— 0.165 | $792.00 |
...
**Annual Facility Revenue = Census Ã— Fee**

| Facility Type | Census | Fee (Integrated) | Annual Revenue |
|---------------|--------|------------------|----------------|
| SNF Example | 50 beds | Ã— $4,583.50 | = $229,175 |
| ALF Example | 50 beds | Ã— $3,699.50 | = $184,975 |
...
**Zero Census Treatment:** Facilities with zero or null census values have been assigned surrogate values based on facility type median to enable revenue calculations. The V19.1 database contains no zero census values (all 20,943 facilities have Census â‰¥ 1).
...
|--------------|------------|------------|------------|
| PCP Only | Census Ã— PCP | Census Ã— (PCP + CCM(adj) + SS(adj)) | Census Ã— (PCP + CCM(adj) + SS(adj)) |
| MH Only | Census Ã— MH(adj) | Census Ã— MH(adj) | Census Ã— (MH(adj) + CCM(adj)) |
| Integrated | Census Ã— TOTAL | Census Ã— TOTAL | Census Ã— TOTAL |

### 4.4 Integration Revenue Formulas

| Service Type | Scenario 1 | Scenario 2 | Scenario 3 |
|--------------|------------|------------|------------|
| PCP Only | Census Ã— (MH(adj) + CCM(adj) + SS(adj)) | Census Ã— MH(adj) | Census Ã— MH(adj) |
| MH Only | Census Ã— (PCP + CCM(adj) + SS(adj)) | Census Ã— (PCP + CCM(adj) + SS(adj)) | Census Ã— (PCP + SS(adj)) |
| Integrated | $0 | $0 | $0 |
| With Barrier | $0 | $0 | $0 |
...
**Formula (All Scenarios):** Census Ã— TOTAL

| Condition | Result |
|-----------|--------|
| Not Served, No Barrier | Census Ã— TOTAL |
...
**Key Insight:** As service packages are enhanced (S1â†’S3), current customer value increases but integration opportunity decreases proportionally. New Business remains constant.

---

## 5. V20.0 Established Baselines
...
| Corporate Facilities | 11,858 |
| Independent Facilities | 9,085 |
| Corporate Chains | 1,007 |
| Barrier Facilities | 891 |
| A_Metro Facilities | 2,043 |
| B_Highway Facilities | 1,251 |
| C_Rural Facilities | 17,649 |
| Metro Assigned Facilities | 1,668 |
...
âœ… New Business Revenue: IDENTICAL across scenarios ($6,915.6M)  
âœ… Integration Revenue: DECREASES S1â†’S2â†’S3 ($154.3M â†’ $144.9M â†’ $140.6M)  
âœ… Current Revenue: INCREASES S1â†’S2â†’S3 ($172.1M â†’ $182.4M â†’ $187.7M)  
âœ… Total Potential: DECREASES S1â†’S2â†’S3 ($7,069.9M â†’ $7,060.6M â†’ $7,056.3M)
...
|-------|----------|--------|
| Database file present | Combined_Database_FINAL_V19_1_Enhanced.xlsx exists | â˜ |
| Scenario files present | All 3 scenario files exist with matching version | â˜ |
| Fee Schedule present | Fee_Schedule_Reference_Vxx.xlsx exists | â˜ |
| Version consistency | All files share same major version number | â˜ |

### 6.2 Database Validation

| Check | Expected | Actual | Pass/Fail |
|-------|----------|--------|-----------|
| Total Facilities | 20,943 | _____ | â˜ |
| SNF Count | 15,244 | _____ | â˜ |
| ALF Count | 5,699 | _____ | â˜ |
| Served Facilities | 1,663 | _____ | â˜ |
| Barrier Facilities | 882 | _____ | â˜ |
| A_Metro Facilities | 2,039 | _____ | â˜ |
| B_Highway Facilities | 1,250 | _____ | â˜ |
| C_Rural Facilities | 17,641 | _____ | â˜ |
| SNF + ALF = Total | True | _____ | â˜ |
| Served + Not Served = Total | True | _____ | â˜ |
| A + B + C = Total | True | _____ | â˜ |

**Census Reasonableness Checks:**

| Check | Expected | Actual | Pass/Fail |
|-------|----------|--------|-----------|
| Zero Census Count | 0 | _____ | â˜ |
| Census Min â‰¥ 1 | True | _____ | â˜ |
| Census Mean | 80-90 beds | _____ | â˜ |
| Census Median | 70-75 beds | _____ | â˜ |

### 6.3 Fee Structure Validation

| Fee Component | Expected | Actual | Pass/Fail |
|---------------|----------|--------|-----------|
| SNF PCP | $3,078.00 | _____ | â˜ |
| SNF MH (adj) | $605.50 | _____ | â˜ |
| SNF CCM (adj) | $108.00 | _____ | â˜ |
| SNF SS (adj) | $792.00 | _____ | â˜ |
| SNF TOTAL | $4,583.50 | _____ | â˜ |
| ALF PCP | $2,084.00 | _____ | â˜ |
| ALF MH (adj) | $715.50 | _____ | â˜ |
| ALF CCM (adj) | $108.00 | _____ | â˜ |
| ALF SS (adj) | $792.00 | _____ | â˜ |
| ALF TOTAL | $3,699.50 | _____ | â˜ |
...
|---------|----------|--------|-----------|
| S1 New Biz = S2 New Biz | True | _____ | â˜ |
| S2 New Biz = S3 New Biz | True | _____ | â˜ |
| S1 Current < S2 Current | True | _____ | â˜ |
| S2 Current < S3 Current | True | _____ | â˜ |
| S1 Integration > S2 Integration | True | _____ | â˜ |
| S2 Integration > S3 Integration | True | _____ | â˜ |

**Scenario 1 Revenue Check:**

| Metric | Expected | Tolerance | Actual | Pass/Fail |
|--------|----------|-----------|--------|-----------|
| Current Revenue | $172.1M | Â±$1M | _____ | â˜ |
| Integration Revenue | $154.3M | Â±$1M | _____ | â˜ |
| New Business Revenue | $6,915.6M | Â±$10M | _____ | â˜ |
| Total Potential | $7,069.9M | Â±$10M | _____ | â˜ |

**Revenue Reasonableness Check (Per Facility):**

| Check | Expected Range | Actual | Pass/Fail |
|-------|----------------|--------|-----------|
| Avg Revenue per Served Facility | $100K - $150K | _____ | â˜ |
| Avg New Biz per Unserved Facility | $300K - $400K | _____ | â˜ |
...
| Check | Expected | Actual | Pass/Fail |
|-------|----------|--------|-----------|
| Barrier facilities with $0 Integration | 882 | _____ | â˜ |
| Barrier facilities with $0 New Business | 882 | _____ | â˜ |
| Served facilities with barriers retain Current Revenue | True | _____ | â˜ |
...
|------|-------------|--------|
| Database Gate | All Section 6.2 checks pass | â˜ |
| Fee Gate | All Section 6.3 checks pass | â˜ |
| Scenario Gate | All Section 6.4 checks pass | â˜ |
| Barrier Gate | All Section 6.5 checks pass | â˜ |
...
- Version: V19.2
- Ready for Report Generation: â˜ Yes â˜ No
...
|---------|------|-------------|
| **V20.0** | Dec 2025 | Major database cleanup: resolved 9 duplicates, added 34 Cardon barriers, standardized 6 corporate names, merged Carrolton entities, geocoded 2 facilities, fixed 13 incomplete records; updated all baselines |
